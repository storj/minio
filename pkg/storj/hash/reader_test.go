...
func TestHashReaderVerification(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("case-%d", i+1), func(t *testing.T) {
			r, err := NewReader(tc.src, tc.size, tc.md5hex, tc.sha256hex, tc.actualSize, false)
			if err != nil {
				t.Fatalf("Test %q: Initializing reader failed %s", tc.desc, err)
			}
			_, err = io.Copy(ioutil.Discard, r)
			if err != nil {
				if err.Error() != tc.err.Error() {
					t.Errorf("Test %q: Expected error %s, got error %s", tc.desc, tc.err, err)
				}
			}
		})
	}
...
func TestHashReaderInvalidArguments(t *testing.T) {
	...
	for i, testCase := range testCases {
		tc := testCase
		t.Run(fmt.Sprintf("case-%d", i+1), func(t *testing.T) {
			_, err := NewReader(tc.src, tc.size, tc.md5hex, tc.sha256hex, tc.actualSize, tc.strict)
			if err != nil && tc.success {
				t.Errorf("Test %q: Expected success, but got error %s instead", tc.desc, err)
			}
			if err == nil && !tc.success {
				t.Errorf("Test %q: Expected error, but got success", tc.desc)
			}
			if !tc.success {
				if err != tc.expectedErr {
					t.Errorf("Test %q: Expected error %v, but got %v", tc.desc, tc.expectedErr, err)
				}
			}
		})
	}
