...
func getAPIErrorResponse(bucket, key string, err APIError, resource, requestID, hostID string) APIErrorResponse {
...
func writeErrorResponse(w http.ResponseWriter, apiError APIError, bucket, key string, reqURL *url.URL) error {
	// Generate error response.
	errorResponse := getAPIErrorResponse(bucket, key, apiError, reqURL.Path,
		w.Header().Get(headerAmzRequestID), "")
	encodedErrorResponse, err := encodeResponse(errorResponse)
	if err != nil {
		return err
	}
	if err := writeResponse(w, apiError.HTTPStatusCode, encodedErrorResponse, mimeXML); err != nil {
		return err
	}
	return nil
}
...
func writeResponse(w http.ResponseWriter, statusCode int, response []byte, mType mimeType) error {
	...
	w.Header().Del(headerSSECKey)
	w.Header().Del(headerSSECopyKey)
	w.Header().Del(headerAmzMetaUnencryptedContentLength)
	w.Header().Del(headerAmzMetaUnencryptedContentMD5)
	if mType != mimeNone {
