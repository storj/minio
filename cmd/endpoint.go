			...
			if IPSet, ok := pathIPMap[endpoint.Path]; ok {
				if !IPSet.Intersection(hostIPSet).IsEmpty() {
					return endpoints, setupType,
						config.ErrInvalidErasureEndpoints(nil).Msgf("path '%s' can not be served by different port on same address", endpoint.Path)
				}
	...
	// Check whether same path is used for more than 1 local endpoints.
	{
		...
		for _, endpoint := range endpoints {
			...
			if localPathSet.Contains(endpoint.Path) {
				return endpoints, setupType,
					config.ErrInvalidErasureEndpoints(nil).Msgf("path '%s' cannot be served by different address on same server", endpoint.Path)
			}
